...
Actualmente expone cuatro funciones:
...
La API pública no reproduce cada fase del pipeline como una operación independiente. Ofrece los puntos de entrada necesarios para reconocer, construir, publicar y desplegar una publicación.
...
`publish()` ensambla en `publish/` los artefactos que ya fueron construidos:
...
La función no renderiza de nuevo. Comprueba la publicación, localiza los directorios de salida declarados por los perfiles y copia los resultados necesarios al destino publicable.

En un multiproyecto puede seleccionarse una o varias publicaciones:
...
El resultado devuelve invisiblemente el plan publicado y registra, entre otros datos:
...
Debe llamarse después de una construcción correcta. Si faltan los artefactos esperados, `publish()` informa del problema en lugar de presentar una publicación incompleta como válida.

## `deploy()`

`deploy()` coordina la construcción y la publicación:

```r
deploy()
```

Su comportamiento es equivalente a:
...
Si `build()` falla, `publish()` no se ejecuta y el directorio `publish/` anterior queda intacto.

`deploy()` no realiza operaciones Git ni publica por sí mismo en un servicio remoto. En la API de `iasi.quarto`, desplegar significa preparar el artefacto local `publish/` mediante un ciclo completo y seguro.
...
## Elegir la operación pública

Cada función pública corresponde a una intención distinta.
...
publish()
  ensambla artefactos ya construidos en publish/

deploy()
  ejecuta build() y, si termina correctamente, publish()
```
